...
// Firebase cross-device sync + hamburger menu + popular 2024/2025 songs

const FIREBASE_URL = "https://devotional-app-default-rtdb.firebaseio.com";

async function firebaseGet(path) {
  try { const r = await fetch(`${FIREBASE_URL}/${path}.json`); return r.ok ? r.json() : null; }
  catch { return null; }
}
async function firebaseSet(path, data) {
  try { const r = await fetch(`${FIREBASE_URL}/${path}.json`, { method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify(data) }); return r.ok; }
  catch { return false; }
}
async function saveSchedule(s) {
  ...
  await firebaseSet('schedule', s);
}
async function loadSchedule() {
  const remote = await firebaseGet('schedule');
  if (remote && remote.preacher) { localStorage.setItem('devotionalSchedule', JSON.stringify(remote)); return remote; }
